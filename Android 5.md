...
说明：1、READ_PHONE_STATE权限非必须权限如果有该权限，并获取到iccid可以判断用户是否切换sim卡并且清除缓存，如果没有该权限，移动认证无法判断用户是否换卡，所以用户换卡后有可能使用原有缓存。
     2、移动认证缓存一般30天有效（默认开启缓存，提升用户体验），建议用户在点切换账户的时候掉用清除缓存的方法。
...
速请求SDK服务端获取 token 和 用户ID(openID) 等信息，提高登录速度，建议掉用者在初始化app首页的时候掉用，可以提高用户登录的体验。
...
本方法用于获取token功能。开发者通过隐式登录方法，无授权弹窗（请各个省公司专业公司添加《中国移动认证服务条款》条款页面地址：https://wap.cmpassport.com/resources/html/contract.html），可获取到token和openID，应用服务端凭token向SDK服务端请求校验。
注：隐式登录返回的token用宇校验用户信息。
说明：如果没有申请 READ_PHONE_STATE 权限，AndroidQ无法获取sim卡信息，所以在AndroidQ我们无法判断用户是否换卡，建议业务方在切换用户登录的时候掉用清除缓存。
...
SDK提供短信验证码作为网关取号的补充功能，获取token。目前短信验证码只支持移动登录。
